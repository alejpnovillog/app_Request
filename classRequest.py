...
# >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
# Obtiene una instancia de un objeto tabla
def foundObject(idtabla):
    ...
    try:
        return AbmTablas(
            **{'ges': GestionRegistros(ambiente=ConfigurarAplicacion.ENV_GX), 'objeto_dal': idtabla, 'dbslate': None})
...
def requestWrk(id, qry, idtabla):
    ...
    try:
        ...
        # el atributo ConfigurarAplicacion.WRK_RECORDS es la cantidad de registros a devolver por default
        else:
            if qry == 0:
...
# >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
# Actualiza los registros de un lote de registros
...
# >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
# Obtenemos un registro de una tabla dependiendo de una condicion
...
# >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
# Obtiene un formato para actualizar
...
# >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
# Realiza la ejecucion de un comando sql    
def requestComand(ambiente, sql, *parametros):
    ...
    try:
        ...
        #retorno = documentos.run_comando(sql)
